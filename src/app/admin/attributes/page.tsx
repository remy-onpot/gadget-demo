...
import { createAttribute, updateAttribute, deleteAttribute } from '@/app/admin/actions'; 
import { Database } from '@/lib/database.types'; 
import { Plus, Trash2, Save, X, Loader2, Tags, Edit2, List, ChevronRight } from 'lucide-react';
...
export default function AttributesPage() {
  ...
  const fetchAttributes = async () => {
    ...
    if (data) {
        setAttributes(data);
        // Set default category to the first available one if empty
  ...
  // Get unique categories and keys for suggestions
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
        try {
            ...
            setFormData({ category: formData.category, key: '', value: '', sort_order: 0 }); 
            fetchAttributes(); 

        } catch (e: unknown) {
            // ✅ FIX: Safe Error Narrowing
  ...
  const openEditor = (attr?: AttributeOption) => {
    if (attr) {
      ...
      setFormData(prev => ({ ...prev, key: '', value: '', sort_order: 0 }));
      setIsEditing(true);
    }
  };
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-200">
           <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
             ...
             <div className="p-6 space-y-5">
                 ...
                 {/* 1. CATEGORY INPUT (With Datalist for suggestions) */}
                 <div>
