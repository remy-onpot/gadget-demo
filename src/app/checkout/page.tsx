...
import { ArrowLeft, User, ShoppingBag, Loader2, Send, ShieldCheck, AlertCircle, Mail } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const handleContactBlur = async () => {
    if (formData.email.includes('@') && checkoutItems.length > 0) {
        console.log("🔫 Sniping abandoned cart...");
        
        // ✅ FIX 1: Map Frontend Item -> Server Action Snapshot
        // We manually reshape the data so the server action (which expects DB columns) receives what it needs.
        const snapshotItems = checkoutItems.map(item => ({
            uniqueId: item.uniqueId,
            quantity: item.quantity,
            variant: item.variant as any,
            // Cast product to 'any' to satisfy the server action's strict DB type requirement
            // We ensure we pass the fields the action actually reads (id, name, base_images)
            product: {
                ...item.product,
                base_images: item.product.images, // Map 'images' (App) -> 'base_images' (DB)
                base_price: item.product.price    // Map 'price' (App) -> 'base_price' (DB)
            } as any 
        }));

        await captureAbandonedCart(formData, snapshotItems);
    }
  };
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const adminPhone = settings['whatsapp_phone']; 
                 ...
                 {checkoutItems.map((item) => (
                   <div key={item.uniqueId} className="flex gap-4">
                      <div className="w-20 h-20 bg-gray-100 rounded-2xl relative overflow-hidden flex-shrink-0 border border-gray-200">
                         {/* ✅ FIX 2: Use item.product.images (Frontend Type) */}
